  ...
  <div class="space-y-3">
    ...
    >
      <div class="text-sm text-slate-600">
        Đang hiển thị
        <span class="font-semibold text-slate-900">Cấp công trình</span>
      </div>
      ...
      <table class="w-full text-left text-sm">
        <thead class="sticky top-0 z-10 bg-slate-50 text-xs text-slate-600">
          <tr>
            ...
            <th class="px-4 py-3">Tên cấp</th>
            <th class="w-40 px-4 py-3">Thứ tự ưu tiên</th>
        ...
        <tbody class="divide-y divide-slate-200">
          ...
          >
            ...
            <td class="px-4 py-3 text-slate-700">{{ row.priority }}</td>
            <td class="px-4 py-3">
              <span
    ...
    >
      <div class="grid gap-3">
        <div>
          <label class="text-xs font-medium text-slate-600">Tên cấp *</label>
          <input
            :value="form.name"
            @input="
              emit('update:form', {
        ...
        <div class="grid grid-cols-1 gap-3 md:grid-cols-2">
          <div>
            ...
            <input
              ...
              class="mt-1 w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm outline-none focus:border-slate-300"
            />
            <p v-if="errors.priority" class="mt-1 text-xs text-rose-600">
              {{ errors.priority }}
            </p>
          </div>
...
defineProps<{
  ...
  errors: Partial<Record<"name" | "priority" | "notes", string>>;
}>();
