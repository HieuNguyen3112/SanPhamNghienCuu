...
export function useUniversityResearchWorkApprovalProvider() {
  ...
  async function loadApprovalListWithFeedback(): Promise<void> {
    await runPageLoad(() => loadList(), {
      loading: {
        title: "Đang tải công trình chờ duyệt",
        message: "Hệ thống đang chuẩn bị dữ liệu xét duyệt cấp trường...",
      },
  ...
  async function loadLookups(): Promise<void> {
    ...
    filtering.academicYearOptions.value = years.map((year) => {
      ...
      if ((year.is_active || year.is_current) && !activeAcademicYearCode) {
  ...
  function resetFilterConditions(): void {
    ...
    filtering.selectedAcademicYear.value = defaultAcademicYearCode.value;
    void loadApprovalListWithFeedback().finally(() => {
  ...
  watch(
    ...
    () => {
      if (!lookupReady.value || suppressFilterReload.value) return;
      void loadApprovalListWithFeedback();
    },
  );

  watch(filtering.selectedLecturerOrResearchWorkKeyword, () => {
    if (!lookupReady.value || suppressFilterReload.value) return;
