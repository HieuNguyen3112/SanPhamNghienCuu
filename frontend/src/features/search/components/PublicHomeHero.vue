<template>
  <section class="relative overflow-hidden bg-[#0b2f54]">
    <div class="absolute inset-0 bg-gradient-to-br from-[#0b2f54] via-[#123e66] to-[#234a74]" aria-hidden="true" />
    <div class="pointer-events-none absolute -left-24 -top-24 h-72 w-72 rounded-full bg-white/10 blur-2xl" aria-hidden="true" />
    <div class="pointer-events-none absolute -bottom-24 -right-24 h-72 w-72 rounded-full bg-rose-500/20 blur-2xl" aria-hidden="true" />
      ...
      <div class="grid gap-8 lg:grid-cols-12 lg:items-center">
        ...
        <div class="lg:col-span-7">
          ...
          <h1 class="mt-4 text-3xl font-extrabold leading-tight tracking-tight text-white md:text-4xl">
            Cơ sở dữ liệu <span class="text-rose-300">Khoa học &amp; Công nghệ</span>
          </h1>
          ...
          <div class="mt-6 grid gap-3 sm:grid-cols-3">
            <div class="flex items-start gap-3 rounded-2xl bg-white/10 p-4">
              <span class="mt-0.5 inline-flex h-9 w-9 items-center justify-center rounded-xl bg-rose-500/20 text-white">📚</span>
              <div>
                <div class="text-sm font-extrabold text-white">Dữ liệu đã phê duyệt</div>
                <div class="mt-1 text-xs text-white/75">Chỉ hiển thị công trình hợp lệ.</div>
              </div>
            </div>
            <div class="flex items-start gap-3 rounded-2xl bg-white/10 p-4">
              <span class="mt-0.5 inline-flex h-9 w-9 items-center justify-center rounded-xl bg-white/15 text-white">🔎</span>
              <div>
                <div class="text-sm font-extrabold text-white">Tìm kiếm theo bộ lọc</div>
                <div class="mt-1 text-xs text-white/75">Giảng viên, khoa, năm học.</div>
              </div>
            </div>
            <div class="flex items-start gap-3 rounded-2xl bg-white/10 p-4">
              <span class="mt-0.5 inline-flex h-9 w-9 items-center justify-center rounded-xl bg-white/15 text-white">🧾</span>
              <div>
                <div class="text-sm font-extrabold text-white">Xem chi tiết/PDF</div>
                <div class="mt-1 text-xs text-white/75">Mở drawer để xem thông tin.</div>
              </div>
            </div>
          </div>

          <div class="mt-7">
            <div class="text-xs font-semibold text-white/80">Nhóm dữ liệu (chọn để tìm)</div>
        ...
        <div class="lg:col-span-5">
          <div class="rounded-2xl bg-white p-5 shadow-lg ring-1 ring-black/5">
            <div class="flex items-center justify-between gap-3">
              <div>
                <div class="text-sm font-extrabold text-slate-900">Tìm kiếm dữ liệu</div>
                <div class="mt-1 text-xs text-slate-500">Chọn nhóm dữ liệu và bấm tìm kiếm.</div>
              </div>
              <span class="inline-flex h-9 w-9 items-center justify-center rounded-xl bg-[#234a74]/10 text-[#234a74]">🔍</span>
            </div>

            <form class="mt-4 space-y-3" @submit.prevent="$emit('search')">
              <div>
                <label class="text-xs font-semibold text-slate-600">Giảng viên (tên/mã)</label>
                <input
              ...
              <div>
                <label class="text-xs font-semibold text-slate-600">Năm học</label>
                <select
...
import type { PublicResearchFilterState, SelectOption } from "@/features/public-research/models/publicResearchModels";
...
function onLecturerQueryInput(e: Event) {
  emit("update-filter", { lecturerQuery: (e.target as HTMLInputElement).value });
}
function onFacultyChange(e: Event) {
  const raw = (e.target as HTMLSelectElement).value;
  emit("update-filter", { facultyId: raw === "" ? null : Number(raw) });
}
function onAcademicYearChange(e: Event) {
  const raw = (e.target as HTMLSelectElement).value;
  emit("update-filter", { academicYearId: raw === "" ? null : Number(raw) });
}
</script>