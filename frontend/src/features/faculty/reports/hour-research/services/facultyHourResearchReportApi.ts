...
} from "@/features/reports/hour_research/hourResearchReportTypes";
...
interface FiltersResponseDTO {
  ...
  scope_faculty?: { id: number; name: string | null };
}
...
export async function fetchFacultyHourResearchReportFilters(): Promise<{
  filters: HourResearchReportFiltersResponse;
  scopeFaculty?: { id: number; name: string | null };
}> {
  const { data } = await http.get<ApiResponse<FiltersResponseDTO>>(
    "/api/faculty/reports/hour-research/filters"
  );
  ...
  return {
    ...
    scopeFaculty: data.data.scope_faculty,
  };
}

export async function fetchFacultyHourResearchReport(params: {
  faculty_id?: number;
  ...
  const { data } = await http.get<ApiResponse<ReportResponseDTO>>(
    "/api/faculty/reports/hour-research",
    { params }
  );
...
export async function exportFacultyHourResearchReportExcel(params: {
  faculty_id?: number;
  academic_year_id?: number;
  status?: HourResearchStatusCode;
}): Promise<ExportResult> {
  return exportReport(
    "/api/faculty/reports/hour-research/export/excel",
    params,
    "hour_research_report.xlsx"
  );
}

export async function exportFacultyHourResearchReportPdf(params: {
  faculty_id?: number;
  academic_year_id?: number;
  status?: HourResearchStatusCode;
}): Promise<ExportResult> {
  return exportReport(
    "/api/faculty/reports/hour-research/export/pdf",
    params,
