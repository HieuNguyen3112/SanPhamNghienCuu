...
class FacultyAuditLogController extends Controller
{
    ...
    public function index(FacultyAuditLogIndexRequest $request)
    {
    ...
    public function meta(FacultyAuditLogLookupsRequest $request)
    {
        $facultyId = $this->resolveFacultyIdForUser($request->user());
        if (! $facultyId) {
            return response()->json([
