...
use App\Http\Requests\Faculty\FacultyResearchReportRequest;
...
class FacultyResearchReportController extends Controller
{
    ...
    public function filters(Request $request)
    {
        ...
        $years = $this->availableYears($facultyId);
    ...
    public function index(FacultyResearchReportRequest $request)
    {
        $facultyId = $this->resolveFacultyIdForUser($request->user());
        if (! $facultyId) {
            return response()->json([
        ...
        if (! empty($validated['department_id']) && ! $this->departmentInScope((int) $validated['department_id'], $facultyId)) {
            return response()->json([
                'message' => 'Cross-faculty access is not allowed.',
            ], Response::HTTP_FORBIDDEN);
        }

        if (! empty($validated['lecturer_id']) && ! $this->lecturerInScope((int) $validated['lecturer_id'], $facultyId)) {
            return response()->json([
                'message' => 'Cross-faculty access is not allowed.',
            ], Response::HTTP_FORBIDDEN);
        }
    ...
    public function exportExcel(FacultyResearchReportRequest $request)
    {
        $facultyId = $this->resolveFacultyIdForUser($request->user());
        if (! $facultyId) {
            return response()->json([
        ...
        $filename = $this->buildExportFilename('faculty_research_report', 'xlsx');
    ...
    public function exportPdf(FacultyResearchReportRequest $request)
    {
        $facultyId = $this->resolveFacultyIdForUser($request->user());
        if (! $facultyId) {
            return response()->json([
        ...
        $result = $this->reportData($validated, $facultyId, false);
        $filename = $this->buildExportFilename('faculty_research_report', 'pdf');
    ...
    private function reportData(array $validated, int $facultyId, bool $paginate): array
    {
        ...
        $summary = $this->buildKpis($filters, $facultyId);
        $charts = $this->buildCharts($filters, $facultyId);
        $table = $this->buildTable($filters, $validated, $facultyId, $paginate);
    ...
    private function availableYears(int $facultyId): array
    {
        $yearExpr = $this->activityYearExpression();

        return $this->baseQueryWithPrimaryMember([], $facultyId, false)
    ...
    private function buildKpis(array $filters, int $facultyId): array
    {
        $baseQuery = $this->baseQueryWithPrimaryMember($filters, $facultyId);
    ...
    private function buildCharts(array $filters, int $facultyId): array
    {
        $categoryExpr = $this->categoryKeyExpression();
        $yearExpr = $this->activityYearExpression();

        $departmentQuery = $this->baseQueryWithPrimaryMember($filters, $facultyId)
            ->select([
        ...
        $donutValues = $this->buildKpis($filters, $facultyId);

        $yearQuery = $this->baseQueryWithPrimaryMember($filters, $facultyId)
            ->selectRaw($yearExpr . ' as activity_year')
            ->selectRaw('COUNT(DISTINCT ra.id) as total_count')
            ->whereRaw($yearExpr . ' is not null')
            ->groupByRaw($yearExpr)
    ...
    private function buildTable(array $filters, array $validated, int $facultyId, bool $paginate): array
    {
        ...
        $query = $this->baseQueryWithPrimaryMember($filters, $facultyId)
    ...
    private function baseQueryWithPrimaryMember(array $filters, int $facultyId, bool $applyFilters = true)
    {
